      ...
      <CardContent sx={{ pb: 1 }}>
        <Stack spacing={1}>
          ...
          <Typography variant="body2" color="rgba(255,255,255,0.72)">
            {[order.size, order.flavor].filter(Boolean).join(' · ') || 'Custom furniture'}
          </Typography>
          {order.customer_name ? (
            <Typography variant="body2" color="rgba(255,255,255,0.72)">
              {order.customer_name}
...
export default function FurnitureOrdersPage() {
  ...
  const deliveryTracking = useModuleGate('delivery_tracking');
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await listCustomOrders({ order_type: 'furniture', per_page: 200 });
      setOrders(res.data || []);
    } catch (err) {
      setError(err.response?.data?.error?.message || 'Failed to load furniture orders');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <PageShell>
      ...
      {deliveryTracking ? (
        <Alert severity="info" sx={{ mb: 2 }}>
          Ready orders move to the Deliveries board for last-mile tracking — use that page to mark delivered.
        </Alert>
      ) : null}
      ...
      {loading ? (
        ...
        <EmptyState
          title="No furniture orders"
          description="Book a custom piece with dimensions, material, advance, and delivery time."
      ...
      <Dialog open={dialogOpen} onClose={() => !updating && setDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>New furniture order</DialogTitle>
        <DialogContent>
