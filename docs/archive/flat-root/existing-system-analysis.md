...
## 3. Current business types (label-only)

Codes in `backend/app/constants/business_types.py`:

`restaurant`, `hotel`, `clothing_store`, `footwear_store`, `kirana_store`, `grocery_store`, `electronics_store`, `retail_shop`, `other`

- Used for registration / profile labeling and FSSAI hints for food types.  
- **Not** wired into industry modules, dashboards, or feature flags.  
- **Medical Store is not present** and must remain **out of scope** for V2.
